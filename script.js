 ...
 */
function play() {
  const rettSvar = randomNumber(1,100);
  let fjöldiAgiskana = 0;
  do{
  gisk = prompt("Veldu tölu á bilinu 1-100");
  parsedGisk = parseGuess(gisk);
  if(parsedGisk === null){
    break;
  } else {
  alert(getResponse(parsedGisk, rettSvar));
  fjöldiAgiskana++;
  }
  } while(parsedGisk !== rettSvar);
  if(parsedGisk !== null) {
    games[win] = fjöldiAgiskana;
    win++;
    getResults;
  }
}
